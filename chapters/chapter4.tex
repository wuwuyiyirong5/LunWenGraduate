...
\chapter{自适应网格上不可压~N-S 方程的代数多重网格预处理策略}
    ...
    \begin{equation}
      \begin{array}{ll}
        \vec{u} = \vec{w},& \mbox{ 在 } \partial \Omega_D \times [0,
        T] \mbox{上},\\
        \nu \displaystyle \frac{\partial \vec{u}}{\partial n} - \mathbf{n} p =
        \vec{0}, & \mbox{ 在 } \partial \Omega_N \times [0, T] \mbox{上},  \\
        \vec{u}|_{t = 0} = \vec{u_0}, & \mbox{在} \Omega \mbox{中}.
      \end{array}
      \label{eq::bc_chap4}
    \end{equation}
    ...
    我们用~\cite{li2001mesh} 和 ~\cite{di2005moving} 中提出的移动网格有限元方法求解~\eqref{eq::NS_chap4} 和 ~\eqref{eq::bc_chap4}。
...
%    进行拼装的时候，只要建立速度单元和压力单元间的索引，便是一个简单的过程。
    ...
    据我们所知，用满足~inf-sup 条件的~$4P_1-P_1$ 元离散可以导出一个鞍点问题，许等(\cite{xu1992iterative}、\cite{shen1999schur})
    将两格子方法应用到~Navier-Stokes 方程的求解。很多人在为~Krylov 子空间方法提供预处理子方面做出了很多工作。
    在~\cite{benzi2005numerical} 中，概括了求解鞍点问题常用的数值方法，例如块预处理和多重网格预处理。文献(\cite{bai2005inexact}, \cite{bai2006structured},\cite{elman2007least},\cite{elman2009boundary} 等)中提出了许多块预处理方法，来
    找到鞍点问题的~Schur 补的更好的近似。\cite{benzi2006augmented} 中提出了求解~Ossen 系统的增广的基于~Lagrange 方法。在~\cite{benzi2011relaxed}
    ...
    在~\cite{elman2014finite} 的基础上，我们将为移动网格有限元方法求解~\eqref{eq::NS_chap4} 和
    \eqref{eq::bc_chap4} 提供了一种代数多重网格预处理策略。通过数值例子来表明，我们预处理子的高效性。
    \section{数值离散}
        \subsection{时间方向的离散}
            ...
            在算子分裂方法，也可以看成是分步方法。最简单的方法是将算子分成两步，在每一步中分别用向前和向后欧拉格式，可以参考~\cite{peaceman1955numerical}。
...
%            \end{algorithmic}
         ...
         两步~Peaceman-Rachford 推广到三步, 保持了当~$t \rightarrow \infty$ 时的数值稳定性：
         \begin{algorithm}
...
%            \end{algorithmic}
         ...
         方程~\eqref{eq::first_step_GL_theta} 。当我们取~$\alpha = \beta = 1/2$ 或者~$\theta = 1 - 1/\sqrt{2}, \alpha + \beta = 1$，当~$t \rightarrow
...
%            \end{algorithmic}
   ...
   \subsection{空间离散}
   根据第 ~$2$ 章中，我们提到的~$4P_1-P_1$ 元，这种有限元是基于两套独立的三角形网格。根据~\cite{li2005multi} 中的几何遗传树，
   速度网格可以通过压力网格全局加密一次得到，如图~\ref{fig::hgeometrytree_chap2} 所示。速度单元和压力单元间的~$1-1$ 对应关系可以根据几何遗传树结构，
   相对容易地获得。我们先给出一些符号的定义：
...
\section{快速~Krylov 求解}
  ...
  为了高效的求解线性方程组~\eqref{eq::linear_system} ，我们用预处理的~GMRES(generalized minimal residual algorithm)作为求解器。在~\cite{elman2014finite}
  ...
  \begin{equation}
    ...
    \right),
  \end{equation}
  其中 ~$S = B_xF^{-1}B_x^T + B_yF^{-1}B_y^T$ 是 ~Schur  补矩阵。在迭代方法中，预处理就是要完成近似矩阵的求逆，实现~$\mathcal{P}^{-1}$ 的过程分为两步：
  第一步是求解~Schur 补的系统，第二步是求解两个与矩阵~$F$ 有关的系统。直接求解~Schur 补的方程组是非常耗时的，因为它系数矩阵里包含一个逆
  矩阵。所以，在实际的计算中，找到~Schur 补的良好近似，然后用近似矩阵代替~Schur 补来进行求解。第 ~$2$ 章中提到~PCD 中近似~Schur  补的矩阵为：
  \begin{equation}
    S_* = A_p F_p^{-1} Q_p,
    \label{eq::PCD}
  \end{equation}
  ...
  \end{equation}
  它其中包含一个质量矩阵~$Q_p^{-1}$ 的求解和一个~Poisson 问题~$A_p^{-1}$ 的求解，
  所以我们可以用为求解泊松问题设计的代数多重网格求解器来实现求解。
  第二步中，因为矩阵~$F$ 是质量矩阵占优的，因此我们也用~Poisson 问题的多重网格求解器去求解
  \begin{equation}
    \begin{aligned}
  ...
  就是迭代的~PCD 方法。我们用的多重网格求解器是在~AFEPack \cite{AFEPack} (一个自适应有限元包)中。在~(\cite{elman2014finite}，
  ...
  通过第 ~$2$ 章中的预处理方法分析知，在出流边界
   ...
   而在~$\Omega_D$ 上，压力~$p_h$ 要满足自然条件 ~$\frac{\partial p_h}{\partial n} = 0$，那就意味着我们在~Dirichlet 边界上，对~$A_p$ 不采取任何操作。
  我们知道对于方腔流，在所有的边界上~$\vec{w}_h \cdot \vec{n} = 0$，所以~\eqref{eq::bc_Fp} 将会退化成
  \begin{equation}
    \frac{\partial p_h}{\partial n} = 0.
  \end{equation}
  这就意味着我们在所有的边界~$\partial \Omega$ 上，对~$F_p, A_p$ 不采取任何操作。在实际的
  计算中，我们在~Neumann 边界上对~$F_p$ 施加与~$A_p$ 相同的操作，即~$p_h = 0$。
  在本文中，我们将~PCD 预处理应用到移动网格有限元方法，来高效地求解~Navier-Stokes 问题~\eqref{eq::linear_system}。
\section{移动网格策略}
  \subsection{网格移动的流程}
     在第~$3$ 章中我们详细的介绍过移动网格的策略，这一章中简要描述一下。在~$t = t_{n}$ 时刻，
     我们获得了~$\mathcal{T}_h^n$ 上的数值解 ~$\vec{u}_h^{(n)}, p_H^{(n)}$。根据~\cite{di2005moving}
     中的方法，用保持散度为~$0$ 的插值方法将~$\mathcal{T}_h^n$ 上的数值解插值到~$t_{n + 1}$ 时刻的网格
     ...
     \item 获取物理网格上的移动方向。求解
       ...
       \end{equation}
     \item 保持散度为~$0$ 的插值。
       在用移动网格有限元方法来求解不可压流体方程的时候，需要在数值解插值的过程中保持散度为~$0$。在~\cite{di2005moving} 中，
       ...
       $x^{\text{old}}, x^{\text{new}}$  是物理区域上的两组坐标。$\tau$ 是一个虚拟时间，因为对流速度~$\delta \vec{x}$
       相对较小，通常取成~$1.0$。这里 ~$\hat{p}$ 是一个临时变量，为了跟~\eqref{eq::NS_chap4} 中的压力变量区别开来。
      ...
      是~$t_n$ 时刻在新网格上的数值解，是一组中间变量。
     \end{enumerate}

   \subsection{用AMG 预处理求解~\eqref{eq::full_discreted_update}}
   对\eqref{eq::full_discreted_update} 数值离散将会得到一个线性系统， 它的系数矩阵为~$\mathcal{M}^p$，定义如下：
   \begin{equation}
     \mathcal{M}^p =
     \left(
       \begin{array}{lll}
   ...
   ~$A_p$ 谱等价的。因此我们可以用 ~$A_p$ 来作为~Schur 补矩阵的近似。那么我们可以选取
   \begin{equation}
     \mathcal{K} =
     \left(
       \begin{array}{lll}
   ...
   最后，为了清楚地表述我们的算法，我们给出流程图如算法 ~\ref{alg::solve_AMG}：
      ...
      \begin{algorithm}
        ...
        \begin{algorithmic}[1]
          \State 用第 3 章中针对~Stokes方程的~AMG 预处理方法，来求解稳态~Stokes 方程，从而获得初值 ~$\vec{u}_h^{(0)}, p_H^{(0)}$。
          \While {$t_n < T$}
...
% ----------------------------------------------------------------------------------------
\section{数值算例}
      我们用三个数值算例来展示我们的方法。在实际的计算中，我们采用稳态~Stokes 方程的解作为~Navier-Stokes 方程的初始值，
      边界条件设置跟~Navier-Stokes 方程一致。在我们的算法中，初始的物理区域和逻辑区域是一致的。我们的代码是基于有限元
      包~AFEPack。
     ...
     \subsection{方腔驱动流}
       ...
       令人满意。在图~\ref{fig::cavity_flow_mesh} 中展示的是发展到稳态的移动网格以及涡量的等高线。我们可以看出网格集中在顶部边和右边边界
       ，这些地方都是涡量比较大的地方。速度的流速线如图~\ref{fig::cavity_flow_streamline}。
       在图~\ref{fig::cavity_flow_divergence} 中，我们对比了均匀网格和移动网格下，速度散度分布。结果发现，速度散度值在移动网格下保持的、
       明显比均匀网格要好。
       \begin{enumerate}
           \item 对比求解 ~\eqref{eq::linear_system} 中~GMRES 的迭代步数。

                在图 ~\ref{fig::cavity_compare_Fp}
                中，在预处理中选用六组不同的 $F_p = \gamma
                A_p + W_h^n$，来对比~GMRES
                的平均迭代步数。当 ~$\gamma = 1$
                时，相对比其它~5
                种情况，需要较少的迭代步数，从而得到一个令我们满意的
                结果。为简便起见，我们将~$F_p = \nu A_p +
                W_h^n$ 中的 ~$\nu$ 去掉。 我们在表
                ~\ref{tab::comparation_GMRES_steps}
                中对比了，用~ AMG-PCD 和 ~ILU
                两种预处理方法求解时，GMRES
                的平均迭代步数以及~CPU
                的计算时间。我们发现~AMG-PCD 预处理比 ~ILU
                更高效。从图
                ~\ref{fig::cavity_compare_move_vs_uni}
                中，可以看出在均匀网格上，AMG-PCD 的~GMRES
                平均迭代次数跟网格尺度无关，但在移动网格上，GMRES
                的迭代次数有所增加，这是因为不同网格尺度的网格移动效果不同。
                但是相比于~ILU，效率还是非常高的。
           \item 对比求解 ~\eqref{eq::full_discreted_update} 中~GMRES 的 迭代步数。
               在表 ~\ref{tab::GMRES_steps_update}中，我们对比不同的预处理矩阵
               ~$M_S^*$ 对应的~GMRES 平均迭代步数。结果发现：在求解 ~\eqref{eq::full_discreted_update}
               时选取 ~$M_S^* = \frac{1}{\nu} A_p$，GMRES 迭代步数相对于 ~$M_S^* = A_p$ 会明显减少。
       \end{enumerate}
       ...
       \begin{figure}[!htbp]
         \begin{center}
             \includegraphics[width = 0.43\textwidth, angle = -90]{picture/second/cavity_flow_data/divergence_uniform.eps}
             \includegraphics[width = 0.43\textwidth, angle = -90]{picture/second/cavity_flow_data/divergence_moving.eps}
        \end{center}
       ...
       \begin{figure}[!htbp]
         \begin{center}
           \includegraphics[width = 0.55\textwidth, angle = 0]{picture/second/cavity_flow_data/it_steps_via_nu.eps}
        \end{center}
        \caption{  方腔流：对比~AMG-PCD 预处理中，选用不同的~$F_p = \gamma A_p + W_h^n$ (其中~$\gamma$ 的值可以变化)，来求解~\eqref{eq::linear_system} 时，GMRES 的平均迭代步数，
       ...
       \begin{table}[!htbp]
         ...
         \begin{tabular}{ccc}
           \hline
           \multirow{2}{*}{压力网格}  & \multicolumn{2}{c}{GMRES 的平均迭代次数(CPU 时间)} \\
           \cline{2-3}
                           & AMG-PCD & ILU \\ \hline
%           $10 \times 10$   &      $9(0.036s)$              &     $43.83(0.122s)$  \\ \hline
           $20 \times 20$   &      $10.8(0.16s)$           &     $104.47(1.974s)$
           \\ \hline
           $40 \times 40$   &      $15.93(0.712s)$             &     $296.29(8.278s)$
           \\ \hline
           $80 \times 80$   &     $27.18$              &     $>500$
           \\ \hline
         \end{tabular}
         \caption{方腔流: 对比用 ~AMG-PCD 和 ~ILU 预处理来求解 ~\eqref{eq::linear_system} 时，GMRES 的迭代步数，$Re = 2000$。}
         \label{tab::comparation_GMRES_steps}
       \end{table}

       \begin{figure}[!h]
         \begin{center}
           \includegraphics[width = 0.55\textwidth, angle = 0]{picture/second/cavity_flow_data/uniform_vs_moving_GMRES_new.eps}
        \end{center}
        \caption{  方腔流：在均匀网格上和移动网格上，求解~\eqref{eq::linear_system} 时，GMRES 的平均迭代次数，$Re = 2000$。}
        \label{fig::cavity_compare_move_vs_uni}
       \end{figure}
...
%       \end{table}
   ...
   \subsection{圆柱绕流}
      ...
      在图 ~\ref{fig::cylinder_mesh_2s} 中，展示了 ~$t = 2s$ 时的网格移动效果。可以发现，网格明显地聚集在圆形障碍物的周围。据我们所知，
