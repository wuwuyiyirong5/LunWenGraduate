%%---------------------------------------------------------------------------------------------------%%
%%------ 第三章：基于4P1-P1元求解不可压Navier-Stokes方程的移动网格有限元方法 ------------------------------------%%
%%---------------------------------------------------------------------------------------------------%%


\chapter{基于$4P_1-P_1$元求解不可压~N-S 方程的移动网格有限元方法}
\label{chapter:4P1-P1_moving}
    ...
    例如 ~Taylor-Hood 元。另外一种方法是对压力空间施加一些约束，
...
%    的工作。
    ...
    ~\cite{bercovier1979error}。在~$4P_1-P_1$ 元中，速度单元所在的网格可以有压
    ...
    只要我们建立四个速度单元和压
    力单元之间的一一索引，散度块矩阵的拼装过程变得很自然。通过我们上一
    章提到的几何遗传树结构，很容易可以建立两层网格间的对应关系。

    基于这种几何遗传树的结构，我们可以很容易的建立四个小的速度单元与
    压力宏单元间的索引。同时在~\cite{li2005multi} 中~$h$ 自适应网格方法也是基
    于这种树结构。自适应网格不仅可以减少计算量，还可以用来研究流体局部小尺
    度上的现象，比如涡。移动网格网格方法也可以应用在~$4P_1-P_1$ 元上，
    我们只需移动压力网格，始终保持速度网格是压力网格的全局一次加密。但是需要注意的是，求解不可压的~Navier-Stokes 方程，
    在将旧网格的数值解插值到新网格的过程中，需要保证散度为~$0$，这个
    工作由~\cite{di2005moving} 中给出。

\section{数据结构}
   $4P_1-P_1$ 是基于两套不同的网格和两种有限元空间。速度网格可以由压力网格
   全局加密一次得到。我们是用~\cite{li2005multi} 中的几何遗
   传树结构来存储这两套网格，如图 ~\ref{fig::hgeometrytree_chap2} 所示。一个宏压力单元对
...
\section{混合元近似}
    \subsection{流体方程}
        ...
        \begin{equation}
            ...
            \label{eq::NS_system}
        \end{equation}
        其中~$\nu > 0$ 是一个常数，称作动力学粘性系数。对流项~$\vec{u} \cdot \nabla \vec{u} := (\vec{u} \cdot \nabla) \vec{u}$，是非
        线性项，这也使得~Navier-Stokes 方程的解不唯一，这也给我们的数值计算带来了一定的挑战。
        系统~\eqref{eq::NS_system} 的计算区域是~$\Omega$，可以是二维的或三维的。边界条件如下：
        \begin{equation}
            \begin{array}{ll}
                \vec{u} = \vec{w},& \mbox{在} \partial \Omega_D,\\
                \nu \displaystyle \frac{\partial \vec{u}}{\partial \vec{n}} - \vec{n} p =
                \vec{0}, & \mbox{在} \partial \Omega_N,  \\
             \end{array}
            \label{eq::bc_chap3}
        \end{equation}
        ...
        \begin{equation}
            ...
            \end{aligned}
        \end{equation}
        如果边界全部是~Dirichlet 边界条件，即~$\partial \Omega = \partial \Omega_D$，那么~Navier-Stokes 问题
        ...
        \begin{equation}
            ...
            \label{eq::streamline}
        \end{equation}
        但由于流函数形式~\eqref{eq::streamline} 在三维空间没有一般形式，所以流函数形式只在二维。我们还是研究~\eqref{eq::NS_system} 中的原始
        变量形式。
    \subsection{弱形式}
        我们定义解和检验空间如下：
        \begin{eqnarray}
       ...
       $\tau_{h}(h = 1/2H)$ 是对速度网格的三角剖分。基于
       ~$\tau_{h}$ 和 ~$\tau_{H}$ 上的有限元空间 ~$\mathbf{X}_E^h$ 和 ~$P^H$ 满足
       \begin{equation}
         \mathbf{X}_E^h \subset \mathcal{H}_E, \quad P^H \subset L_2(\Omega).
         \notag
       \end{equation}
   ...
   \begin{equation}
     \begin{aligned}
       &\int_{\Omega} \frac{\vec{u}_h^{(n + 1)} -
         \vec{u}_h^{(n)}}{\delta t} \cdot \vec{v}_h \\
        & + \nu \int_{\Omega} \nabla \vec{u}_h^{(n + 1)} : \nabla \vec{v}_h
         - \int_{\Omega} p_H^{(n
   ...
   力的线性元基函数。则数值解~$\vec{u}_h^{(n + 1)} = (u_{x,h}^{(n + 1)},
   u_{y,h}^{(n + 1)})^T, p_H^{n + 1}$ 可以写成如下形式：
   \begin{equation}
     u_{x,h}^{(n + 1)} = \sum_{j = 1}^n \alpha_{x,j} \phi_j, \quad u_{y,h}^{(n +
       1)} = \sum_{j = 1}^n \alpha_{y,j} \phi_j, \quad p_H^{(n + 1)} = \sum_{k
       = 1}^m \alpha_{p,k} \psi_k.
       \label{eq::solutions_components}
   \end{equation}
   将 \eqref{eq::solutions_components} 带入离散弱形式
   ...
   \end{equation}
   其中 $\alpha_x = (\alpha_{x,1},\cdots,
    \alpha_{x,n_u}),\alpha_y = (\alpha_{y,1},\cdots, \alpha_{y,n_u}),\alpha_p = (\alpha_{p,1},\cdots, \alpha_{p,n_p})$，
    ~$dt$ 是时间步长(需满足~CFL 条件)，~$M$ 是 ~$n \times n$ 的质量矩阵，$A$ 是拉普拉斯矩阵，由以下形式：
   \begin{eqnarray}
     ...
     f_x & = & [f_i], \quad f_i = \int_\Omega (\frac{u_{x,h}^{(n)}}{dt} - (u_{x,h}^{(n)};
     \frac{\partial u_{x,h}^{(n)}}{\partial x} + u_{y,h}^{(n)} \frac{\partial
       u_{x,h}^{(n)}}{\partial y}) \phi_i; \notag \\
     f_y & = & [f_i], \quad f_i = \int_\Omega (\frac{u_{y,h}^{(n)}}{dt} - (u_{x,h}^{(n)}
     \frac{\partial u_{y,h}^{(n)}}{\partial x} + u_{y,h}^{(n)} \frac{\partial
       u_{y,h}^{(n)}}{\partial y}) \phi_i; \notag \\
     g & = &  [g_k],\quad  g_i = 0. \notag
   \end{eqnarray}

   接下来我们着重解释一下散度块矩阵~$B_x, B_y, B_x^T, B_y^T$ 的拼装。 以
   ~$B_x^T$ 为例, 令~$\triangle_{v_i}$ 为一个速度单元，我们可以通过上一节建立的
   单元索引，来找到对应的压力单元 ~$\triangle_{p_k}$(如图~\ref{fig::matrix_assemble})。$\phi_{i_1}, \phi_{i_2}, \phi_{i_3}$
   ...
   \end{figure}
   \section{预处理策略}
      在我们求解~Navier-Stokes 方程的算法中，用稳态~Stokes 方程的解作为初始值，因此在这一部分中，我们考虑针对
      ~Stokes 方程的预处理方法。我们写出~\eqref{eq::stokes_chap3} 的~$4P_1-P_1$ 近似的离散弱形式：
      寻找 ~$(\vec{u}_h, p_h) \in X_E^h \times P^H$
      使得
       \begin{equation}
         \begin{aligned}
           \nu \int_\Omega \nabla \vec{u}_h : \nabla \vec{v}_h
      ...
      ~$\vec{u}_h^{(n + 1)} = (u_{x,h}^{(n + 1)}, u_{y,h}^{(n + 1)})^T, p_H^{(n + 1)}$ 可以写成如下形式：
      \begin{equation}
         u_{x,h}^{(n + 1)} = \sum_{j = 1}^n \beta_{x,j} \phi_j, \quad u_{y,h}^{(n +
         1)} = \sum_{j = 1}^n \beta_{y,j} \phi_j, \quad p_H^{(n + 1)} = \sum_{k
         = 1}^m \beta_{p,k} \psi_k.
         \label{eq::solutions_components_stokes}
      \end{equation}
      将 \eqref{eq::solutions_components_stokes} 带入离散弱形式
      ~\eqref{eq::time_discreted_weak}  中，可以
      得到线性方程组
       \begin{equation}
         \left[
     ...
     我们要为~MINRES 选择一个有效的预处理子。从上一章中可知，压力空间的质量矩阵:
     \begin{equation}
        Q_p  =  [Q_{p,ij}], \quad q_{ij} = \int_\Omega \nabla \psi_i \cdot \nabla \psi_j,  \notag
     \end{equation}
     可以作为~Schur 补~$S = B_x A^{-1} B_x^T + B_y A^{-1} B_y^T$ 的近似矩阵。
     在实际的计算中，压力质量矩阵的对角阵~$\hat{Q_p} = diag(Q_p)$，就能
     有比较好的效果。因此，我们选取对角块矩阵~$K$
       \begin{equation}
         K = \left[
             ...
             \right],
       \end{equation}
       作为 ~\eqref{eq::linear_system_stokes} 中系数矩阵的近似。
%       \begin{equation}
%         M = \left[
%               \begin{array}{lll}
%                 \nu A & 0 & B_x^T \\
...
%       \end{equation}
   ...
   其中~$V_{dst} = (V_{dst}^x, V_{dst}^y, V_{dst}^p)^T$ 为未知量，$V_{src} = (V_{src}^x,
   V_{src}^y, V_{src}^p)^T$ 为已知量。
   我们不是直接求解 ~$V_{dst} = K^{-1} V_{src}$，而是把~$K$ 分成三个对角块 ~$(0, 0)$，$(1, 1)$ 和 ~$(2, 2)$ 块，
   然后每个块进行单独求解。我们用~Poisson 问题的代数多重网格求解器(AFEPack ~\cite{AFEPack} 中的~AMG 求解器)来求解
   \begin{eqnarray}
...
\section{移动网格策略}
   ...
   虽然我们用的是两套网格，即速度网格和压力网格，但是速度网格可以由压力
   网格加密一次得到。因此我们只移动压力网格，然后始终保持速度网格是压力网格的
   全局一次加密，就可以实现速度网格的同步移动。

   在 ~$t = t_{n}$ 时刻，获得有限元解~$(\vec{u}_h^{(n)}, p_H^{(n)})$。下面的问题是如何根据~$t_n$ 时刻数值
   解和旧网格~$\mathcal{T}_H^{(n)}$ 来获得新的网格~$\mathcal {T}_H^{(n +
   ...
   \subsection{获取控制函数}
     ...
     可压~Navier-Stokes 方程上的网格控制函数~$G$ 主要有如下几种：
     一种是基于涡量
     \begin{equation}
   ...
   \subsection{获取新的逻辑网格}
     ...
     \end{eqnarray}
     其中纯量函数~$m = \frac{1}{G}$。我们定义初始的逻辑网格~$\mathcal{T}_c^{0} (\mathcal
     {A}^{0} \text{为它的节点})$。一旦初始的逻辑网格给定，在整个的求解
     过程中，将一直保持不变。通过求解~\eqref{eq::logical_chap3}  我们可以得到
     新的逻辑网格剖分~$\mathcal{T}_c^*$($\mathcal{A}^*~$ 为它的节点)。
  \subsection{物理网格的移动方向}

     我们先引入一些定义。$\mathcal{T}_H$ 为压力网格物理区域上的三角剖分。第~$i$ 个
  ...
  求解该方程组，可以获得单元 ~$E$ 上的~$\partial \vec{x} / \partial
  ...
  \subsection{散度为~$0$ 的插值}
    ...
    u_h \in \mathbf{X}_E^h$， $\phi_i$ 是有限元空间~$\mathbf{X}_E^h$ 的
    ...
    \begin{equation}
      \mathbf{X}^h = \mathbf{X}_E^h \cap \{\vec{u}_h|\nabla \cdot \vec{u}_h
      = 0 \},
    \end{equation}
   ...
   \end{algorithm}
    ...
    \section{数值算例}
       \subsection{碰撞流}
          ...
          首先从
          图~\ref{fig::mesh_move_direction} 中看出网格是从控制函数大的地方移动到控制函数值小的地方；
          从图~\ref{fig::uniform_vs_moving_err} 中看到：相比于均匀网格，移动网格
          能够明显的降低速度~$L^2$ 误差。
          \begin{figure}[!htbp]
            \begin{center}
     ...
     \subsection{喷射流}
       ...
       这个算例模拟的是一个很细的流体喷射入一个静止的流场，这种现象在~\cite{milton1982album}中有描述，
        一本有趣的画册。我们的计算区域是~$\Omega = [0, 12] \times [-3, 3]$ 并且粘性系数是~$\nu = 0.0005$。自然条件设置在出流边界~$x = 12$ 上，
   ...
   \subsection{流体经过一个凹形台阶}
      ...
      我们考虑~\cite{zheng2010adaptive} 中的算例，N-S 流体以~$Re = 1000$ 经过一个凹形的台阶，计算区域是
