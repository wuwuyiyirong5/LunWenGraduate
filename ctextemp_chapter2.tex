...
\section{混合有限元}
    ...
    L_0^2(\Omega)$, $L_0^2 = \{q \in L^2(\Omega)| \int_{\omega}q dx = 0$。其中有限元空间
...
\section{移动网格方法}
    ...
    Li \cite{li2001moving} 中提出一种移动有限元方法，它是基于调和映射理论的。主要想法是先选取一个初始的逻辑
    ...
    \end{equation}
    来获得一个初始的逻辑网格 ~$\xi^0$，相当于控制函数全部取 ~$1$。这个初始逻辑网格获得之后，在后面的计算中保持不变。
   \begin{algorithm}
     \caption{网格重新分布}
     \begin{algorithmic}[1]
        \State 给出物理网格 ~$\vec{x}^{(n)}$ 上的数值解~$\vec{u}^{(n)}$
               并通过求解 ~\eqref{eq::harmonic_map_eq}, 获得~$\vec{\xi}^*$. \label{state::monitor}
        ...
        \State 用~$\vec{\xi}^* - \vec{\xi}^{(0)}$ 两者之差来计算
               ...
               \label{state::start}
        \State 利用~\ref{state::start} 中~$\delta \vec{x}$, 在网格
     ...
     \label{alg::solve}
   \end{algorithm}
   在将旧网格上的数值解差值到新网格上的过程中，Li \cite{li2001mesh} 提出了一种保持数值解 ~$L_2$ 范数的同伦方法。
   Di \cite{di2008general} 和 ~Wang \cite{Wang2008} 应用代数多重网格方法求解 ~\eqref{eq::harmonic_map_eq}。需要指出的
   是这种移动有限元方法将网格的移动和物理方程的求解分别开来，从而使得程序写起来非常方便，针对不同的物理问题，
   提供不同的控制函数，以及新网格更新数值解的部分程序。例如~Di 在 ~\cite{di2005moving} 中在求解不可压流体问题时，需要在数值解插值的过程中，
   保证数值解的散度为 ~$0$。上面的算法 ~\ref{alg::solve} 已经在写在了自适应有限元软件包~AFEPack(\cite{AFEPack}) 中。
 \section{预处理方法}
    ...
    矩阵 ~$M^{-1}K$ 和 ~$M^{-1}\vec{r}^{(0)}$ 张成的~Krylov 子空间
    \begin{equation}
...
%        \floatname{algorithm}
    ...
    可以参考 ~\cite{saad2003iterative}、\cite{van2003iterative} 等。同样的，在求解非对称的不定矩阵的时候，如
    ...
    \subsection{针对~Stokes 方程}
        ...
        在 ~\cite{elman2014finite} 中指出，在不影响~MINRES 收敛的前提下，可以考虑用比 ~$T = \mbox{diag}(Q)$ 来作为~Schur 补矩阵的近似。并给出了 ~$T = Q$ 和 ~$T = \mbox{diag}(Q)$ 的界
        \begin{lemma}
        ...
        \begin{equation}
            a(u, v) = \mathcal{l}(v) \mbox{对所有的} v \in \mathcal{H}_{E_0}^1.
        ...
        \begin{theorem}
            ...
            为 ~$h$ 的拟一致剖分，并且用一个 ~$l$ 层多重网格~W 循环来求解 ~$A\vec{u} = \vec{f}$。那么
        ...
        \begin{theorem}
            ...
            为 ~$h$ 的拟一致剖分，并且用一个对称的 ~$l$ 层多重网格~V 循环来求解 ~$A\vec{u} = \vec{f}$。那么
        ...
        \begin{proof}
            ...
            对所有的 ~$\vec{v}$。
        \end{proof}
        结合 ~\ref{theo::multigrid_V}，\ref{theo::multigrid_W} 和 ~\ref{lemma::bound_P_A}，我们可以用一个多重网格循环来
        代替 ~$\vec{P}$。如果在预处理中我们选择 ~$\vec{P} = \vec{A}$，并且用一次多重网格~V 型或~W 型循环代替
         ~$P$，虽然~MINRES 迭代次数会额外增加一些，但是是与 ~$h$ 无关的。但是用对角块直接做预处理矩阵，MINRES 的迭代步数
        会随着 ~$h$ 的减小而增加。
    ...
    \subsection{针对~Navier-Stokes 方程}
        ...
        \end{equation}
        其中 ~$vec{Q}$ 是速度质量矩阵，$\vec{F}_p$ 和 ~$F_p$ 分别是速度和压力空间的对流扩散矩阵。
        $F_p$ 为：
        \begin{equation}
        ...
        谱等价的。在求解入流/出流问题时，对压力空间刚度矩阵 ~$A_p$ 和 压力对流扩散矩阵 ~$F_p$ 不做
        ...
        \end{equation}
        它的弱形式为：找到 ~$(\vec{u},p_h) \in X_E^h \times M^h$ 使得
        \begin{equation}
            \int_{\nabla \vec{u}_h} : \nabla \vec{v}_h - \int_{\Omega}p_h \nabla \cdot \vec{v}_h  = \int_{\partial \Omega_N} \vec{s} \cdot \vec{v}_h \quad \mbox{对所有的} \vec{v}_h \in X_0^h,
            \label{eq::weak_form_stokes_momentum}
        \end{equation}
        \begin{equation}
            \int_{\Omega} q_h \nabla \vec{u}_h  = 0 \quad \mbox{d 对所有的} q_h \in M^h.
        ...
        另外一种预处理方法是最小二乘交换子预处理~(LSC precodition)，我们做如下简要介绍：
        \begin{equation}
            M_S = (B\vec{T}^{-1}B^T)(B\vec{T}^{-1}\vec{F}\vec{T}^{-1}B^T)(B\vec{T}^{-1}B^T).
        \end{equation}
        ...
        对比这两种预处理方法，PCD 预处理需要额外的生成两个矩阵 ~$A_p$ 和 ~$F_P$，但是这种方法可以直接
        应用到稳定化的混合元近似 ~($\vec{Q}_1-\vec{Q}_1$ 和 ~$\vec{Q}_1-\vec{P}_0$)。LSC 预处理不需要额外生成矩阵，但是在每步
        中需要两次泊松求解。